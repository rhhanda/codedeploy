...
class CloudWatchLogsAgentSetup:
    ...
    def __init__(self, log_file, options):
        ...
        self.ispipinstalledbyscriptforpython26 = False
    ...
    def get_distro_info(self):
        ...
        if os.path.isfile(os_release):
            with open(os_release, "r") as os_release_file:
                line = os_release_file.readline()
    ...
    def install_awslogs_cli(self):
        ...
        if self.is_standalone:
            ...
            # the virtual env.
            message("Standalone installation, setting AWSLOGS_HOME as the virtualenv ... \n")
            self.validate_dependency_path(self.dependency_path)
            self.setup_virtualenv()
            message("Virtualenv set.\n")
        else:
            # No local dependency folder provided, install virtualenv 
            # from pip.
            message("Not standalone installation, installing virtualenv from pip ... \n")
            self.do_pip_install('virtualenv', False)
            message("Virtualenv installed.\n")
        ...
        if not os.path.exists(VIRTUALENV_ACTIVATE_CMD):
            ...
            message("Checking virtualenv path ... \n")
            ...
            message("Appending --python to virtualenv commands ... \n")
            if self.python:
        ...
        if self.is_standalone:
            ...
            message("Installing awscli-cwlogs 1.4.4 ... \n")
            ...
            if self.plugin_url:
                if need_wheel_29:
                    message("Installing wheel 0.29.0 ... \n")
        ...
        if not self.ca_bundle == "":
            message("Configuring the default CA bundle ... \n")
    ...
    def write_agent_config_file(self):
        message("Attempting to manually create/locate the agent config file ... \n")
    ...
    def do_pip_install(self, package_descriptor, virtual_env_pip=True, folder_path=None, do_upgrade=False, extra_index_url=None):
        ...
        if not virtual_env_pip:
            if self.ispipinstalledbyscriptforpython26 and executable_exists("pip2.6"):
                pip_path= 'pip2.6'
            else:
                pip_path = 'pip'
    ...
    def install_pip(self):
        ...
        # if it is not standalone, in some case, we need to install the pip from the scripts.
        # For example: ubuntu 12, it has python 2.7 but it has pip 1.0, it can not install virtualenv by "pip install virtualenv"
        # On centos 6, the python is 2.6, it can not install pip with "easy_install pip"
        if not self.is_standalone:
            if self._need_install_pip_from_script():
                message("Installing pip from https://s3.amazonaws.com/aws-cloudwatch/downloads/get-pip/get-pip.py")
                subprocess.call(["wget", "https://s3.amazonaws.com/aws-cloudwatch/downloads/get-pip/get-pip.py"], stdout=self.log_file, stderr=self.log_file)
                subprocess.call(["python", "get-pip.py"], stdout=self.log_file, stderr=self.log_file)
                self.ispipinstalledbyscriptforpython26 = True
                message("Installed pip from https://s3.amazonaws.com/aws-cloudwatch/downloads/get-pip/get-pip.py")
    ...
    def _need_install_pip_from_script(self):
        if not executable_exists("pip"):
            python_version = sys.version_info
            if python_version >= (2, 6) and python_version < (2, 7):
                return True
        else:
            pip_version = subprocess.Popen(['pip', '--version'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[ 0].rstrip().lower()
            if pip_version < "pip 6.0":
                return True
        return False
    ...
    def setup_artifacts(self):
        ...
        if not self.non_interactive:
            ...
            message("Non interactive installation, skipping Steps 3 & 4 of 5 \n")
